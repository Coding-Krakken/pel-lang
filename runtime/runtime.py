...
class PELRuntime:
    ...
    def run_deterministic(self, ir_doc: dict[str, Any]) -> dict[str, Any]:
        ...
        # Time loop
        timeseries_results = {
            node["name"]: [] for node in model["nodes"] if node["node_type"] == "var"
        }
    ...
    def evaluate_expression(
        self, expr: dict[str, Any], state: dict[str, Any], deterministic: bool = True
    ) -> Any:
...
def main():
    """CLI entry point."""
    parser = argparse.ArgumentParser(description="PEL Runtime - Execute compiled PEL-IR models")

    parser.add_argument("ir_file", type=Path, help="Compiled .ir.json file")
    parser.add_argument(
        "--mode",
        choices=["deterministic", "monte_carlo"],
    ...
    config = RuntimeConfig(
        mode=args.mode, seed=args.seed, num_runs=args.runs, time_horizon=args.time_horizon
    )
