...
def test_burn_rate():
    ...
    src = (
        ...
        "  var burn_rate: Rate per Month = total_burn / months\n"
        "}\n"
    )
    tokens = Lexer(src).tokenize()
    model = Parser(tokens).parse()
...
def test_operating_cash_flow():
    ...
    src = (
        ...
        "    confidence: 1.0\n"
        "  }\n"
        "  var ocf: Currency<USD> = net_income + depreciation - wc_change\n"
        "}\n"
    )
    tokens = Lexer(src).tokenize()
    model = Parser(tokens).parse()
...
def test_payroll_timing():
    ...
    src = (
        ...
        "    confidence: 1.0\n"
        "  }\n"
        "  var monthly_payroll: Rate per Month = (headcount * annual_salary) / 12mo\n"
        "}\n"
    )
    tokens = Lexer(src).tokenize()
    model = Parser(tokens).parse()
