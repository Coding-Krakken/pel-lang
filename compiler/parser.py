...
class Parser:
    ...
    def parse_model_item(
        self,
    ) -> ParamDecl | VarDecl | FuncDecl | Constraint | Policy | Statement:
    ...
    def parse_statement(self) -> Statement:
        ...
        self.consume_if(TokenType.SEMICOLON)
        return Assignment(
            target=Variable(name="_"), value=Literal(value=0.0, literal_type="number")
        )
    ...
    def parse_type(self) -> TypeAnnotation:
        ...
        # User-defined types (identifier)
            ...
            raise syntax_error(
                f"Expected type, got {self.current().type.name}", self.current_location()
            )
    ...
    def parse_primary_expression(self) -> Expression:
        ...
        # Identifier or function call
            ...
            raise syntax_error(
                f"Expected expression, got {self.current().type.name}", self.current_location()
            )
    ...
    def parse_correlation_pair(self) -> tuple:
        ...
        varname = (
            ...
            if self.match(TokenType.IDENTIFIER)
            else self.expect(TokenType.STRING).value
        )
        self.expect(TokenType.COMMA)
