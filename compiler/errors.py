...
def type_mismatch(expected: str, got: str, location: SourceLocation | None = None) -> TypeError:
    """E0100: Type mismatch."""
    return TypeError("E0100", f"Type mismatch: expected {expected}, got {got}", location)
...
def dimensional_mismatch(
    op: str, left: str, right: str, location: SourceLocation | None = None
) -> DimensionalError:
...
def currency_mismatch(
    currency1: str, currency2: str, location: SourceLocation | None = None
) -> DimensionalError:
...
def rate_unit_mismatch(
    unit1: str, unit2: str, location: SourceLocation | None = None
) -> DimensionalError:
...
def cyclic_dependency(
    var_name: str, cycle: str, location: SourceLocation | None = None
) -> CausalityError:
...
def missing_provenance_field(
    param_name: str, field: str, location: SourceLocation | None = None
) -> ProvenanceError:
...
def invalid_confidence(value: float, location: SourceLocation | None = None) -> ProvenanceError:
    ...
    return ProvenanceError(
        "E0402", f"Invalid confidence: {value} (must be in [0.0, 1.0])", location
    )
...
def invalid_constraint_condition(
    msg: str, location: SourceLocation | None = None
) -> ConstraintError:
...
def contradictory_constraints(
    constraint1: str, constraint2: str, location: SourceLocation | None = None
) -> ConstraintError:
...
def invalid_distribution_param(
    dist_type: str, param: str, reason: str, location: SourceLocation | None = None
) -> DistributionError:
    """E0600: Invalid distribution parameter."""
    return DistributionError(
        "E0600", f"Invalid {dist_type} parameter '{param}': {reason}", location
    )


def invalid_correlation(
    var1: str, var2: str, coef: float, location: SourceLocation | None = None
) -> DistributionError:
...
def unexpected_token(expected: str, got: str, location: SourceLocation | None = None) -> ParseError:
    ...
    return ParseError("E0700", f"Expected {expected}, got {got}", location)


def syntax_error(msg: str, location: SourceLocation | None = None) -> ParseError:
    """E0701: Generic syntax error."""
    return ParseError("E0701", f"Syntax error: {msg}", location)
